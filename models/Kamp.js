...
dbGetCompletedCamp = async () => {
    const sql = `SELECT ime_kamp, datum_odrzavanja_kamp, 
    trajanje_d, pocetak_prijava_sudionika, kraj_prijava_sudionika, 
    pocetak_prijava_animatora, kraj_prijava_animatora, broj_grupa,
    status, email_kamp
    FROM kamp
    WHERE CURRENT_TIMESTAMP(0) BETWEEN (datum_odrzavanja_kamp + ( trajanje_d * interval '1 day'))
            AND (datum_odrzavanja_kamp + ( (trajanje_d+7) * interval '1 day')) `;
    try {
        const result = await db.query(sql, []);
        return result.rows;
    } catch (err) {
